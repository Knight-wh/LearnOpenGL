...
float sceneSDF(vec3 p) {
  // * 通过给采样点p进行平移和旋转，则可以实现对SDF的改变。比如对于一个球体，使其变大的方法
  // * 有两种，一种是改变sphereSDF的r(符合直觉)，另一种是对p进行逆变换，可以实现同样的效果。
  // * float dist = someSDF(samplePoint / scalingFactor) * scalingFactor; 
  // * 后面的scalingFactor是为了弥补缩放导致的距离变化，参考见readme
  float sphereDist = sphereSDF(p / 1.5 , 1.2) * 1.5; // * same as sphereSDF(p , 1.2 * 1.5);
  // float cubeDist = boxSDF(p, vec3(1.0, 1.0, 1.0));
  // float sdField = intersectSDF(sphereDist, cubeDist);
  float sdField = sphereDist;
  return sdField;
}
...
void main() {
  ...
  mat4 viewMat = viewMatrix(cameraPos, lookAtPos, cameraUp);
